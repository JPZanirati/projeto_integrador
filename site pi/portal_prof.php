...
<head>
  ...
  <link rel="stylesheet" href="css/entrou_prof.css" />
...
<body>
  ...
  <div class="Fundo_tabela">
    ...
    <div class="Tabela">
      ...
      <form method="POST" action="turma_prof.php">
        <input type="hidden" name="id_prof" value="<?php echo $_SESSION['logado_prof'] ?>">
      ...
      <table class="tabela_turmas">
        ...
        <tbody>
          ...
          try {
            ...
            $query = $conn->prepare("SELECT pi_aluno.id_aluno, pi_turma.id_turma, pi_nota.id_nota, nome_aluno, idioma, nome_prof, nota_um, nota_dois FROM pi_aluno 
              join pi_turma using (id_turma) 
              join pi_prof using(id_turma) 
              left join pi_nota using(id_aluno)
              where pi_prof.id_prof = ?");
            $query->execute(array($_SESSION['logado_prof']));
            if ($query->rowCount() > 0) {
              $resultado = $query->fetchAll(PDO::FETCH_OBJ);
              foreach ($resultado as $indice => $prof) {
                echo '<tr>
                <td>' . $prof->idioma . '</td>
								<td>' . $prof->nome_aluno . '</td>
								<td>' . $prof->nota_um . '</td>
                <td>' . $prof->nota_dois . '</td>
                <td>' . ($prof->nota_um + $prof->nota_dois) / 2 . '</td>
								<td class="text-center">
								<a href="edita_nota.php?id_nota=' . $prof->id_nota . '&id_turma=' . $prof->id_turma . '&id_aluno=' . $prof->id_aluno . '" title="Editar"><i class="fa fa-pencil" style="color:rgb(173, 208, 240)"></i></a>';
              }
              echo '</td></tr>';
              echo "</table>";
            } else {
      ...
      <form method="POST" action="logout_prof.php">
